...
function posRenderMovimientosCaja(){var movs=window.MOVIMIENTOS_FINANCIEROS_POS||[];return '<div class="pos-section-title pos-mov-title">Movimientos de caja recientes</div><div class="pos-lista">'+movs.map(function(m){var origen=m.referenciaTipo||m.origenTipo||'movimiento',titulo=origen==='reparacion'?'Cobro reparación '+(m.orden||''):(m.tipo==='reversion_venta_equipo'?'Anulación de equipo '+(m.equipoModelo||''):(origen==='venta_equipo'?'Venta de equipo '+(m.equipoModelo||''):(m.tipo==='reversion_venta'?'Anulación de venta':'Venta de accesorio')));return '<div class="pos-list-row"><span><b>'+esc(titulo)+'</b><small>'+posFecha(m.fechaHora)+' · '+esc(m.medio||'')+' · '+esc(m.cuenta||'')+(m.clienteNombre?' · '+esc(m.clienteNombre):'')+(m.moneda==='ARS'&&m.cotizacion?' · TC '+posDinero(m.cotizacion,'ARS'):'')+'</small></span><b class="'+(posNumero(m.monto)<0?'pos-out':'pos-in')+'">'+posDinero(m.monto,m.moneda)+'</b><small>'+esc(m.tipo||'')+'</small></div>';}).join('')+'</div>';}
