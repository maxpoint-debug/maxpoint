...
function posElegirReparacionCobro() {
  var pendientes=(window.REPS||[]).filter(function(r){return r.estado!=='Entregado'&&r.estado!=='No aprobado'&&saldoReparacion(r)>0;}).sort(function(a,b){return String(b.orden||'').localeCompare(String(a.orden||''),undefined,{numeric:true});}).slice(0,80);
  var contenido='<div class="pos-section-title">Reparaciones con saldo</div><div class="pos-lista">'+pendientes.map(function(r){return '<button class="pos-producto" onclick="posAbrirCobroDesdeCaja(\''+r.id+'\')"><span><b>'+esc(r.orden||'Orden')+' · '+esc(r.nombre||'')+'</b><small>'+esc(r.equipo||'')+'</small></span><span><b>'+pesos(saldoReparacion(r))+'</b><small>Cobrar</small></span></button>';}).join('')+'</div>';
  if(!pendientes.length)contenido='<div class="pos-vacio">No hay reparaciones abiertas con saldo pendiente.</div>';
  posModal('Cobrar reparación',contenido,posCerrarModal,'Cerrar');
  var cancelar=document.querySelector('#posModal .mf>.btn-g');if(cancelar)cancelar.style.display='none';
}
function posAbrirCobroDesdeCaja(id){posCerrarModal();openPago(id);}

function renderPos() {
  var cnt=el('cnt'), c=posCalculos(), moneda=posMonedaCarrito(), clientes={}, opcionesClientes=''; cnt.innerHTML='';
  if(typeof cajaActualAbierta==='function'&&!cajaActualAbierta()){cnt.innerHTML=cajaEstadoHtml();return;}
...
function renderPosHistorial(){var cnt=el('cnt'),vs=posVentas();cnt.innerHTML=posRenderResumenCaja()+'<div class="pos-section-title pos-mov-title">Ventas de accesorios</div><div class="pos-lista">'+vs.map(function(v){return '<div class="pos-list-row pos-venta-row"><span><b>'+esc(v.numeroHumano||('Venta #'+v.numeroVenta))+'</b><small>'+posFecha(v.fechaHora)+' · '+esc((v.cliente&&v.cliente.nombre)||'Consumidor final')+' · '+esc((v.pagos||[]).map(function(p){return p.medio;}).join(' + '))+' · '+esc(v.moneda||'ARS')+'</small></span><b class="'+(v.estado==='anulada'?'pos-anulada':'')+'">'+posDinero(v.total,v.moneda)+'</b><span><button class="btn btn-g btn-sm" onclick="posVerVenta(\''+v.id+'\')">Ver / ticket</button>'+(v.estado==='activa'&&puede('anular_venta_pos')?' <button class="btn btn-d btn-sm" onclick="posAnularVenta(\''+v.id+'\')">Anular</button>':'')+'</span></div>';}).join('')+'</div>'+posRenderMovimientosCaja();}

function posRenderResumenCaja(){var movs=(window.MOVIMIENTOS_FINANCIEROS_POS||[]).filter(function(m){return posEsHoy(m.fechaHora);}),sumas={},cuentas={},operaciones={};movs.forEach(function(m){var moneda=m.moneda||'ARS',monto=posNumero(m.monto);sumas[moneda]=(sumas[moneda]||0)+monto;var k=(m.medio||'Sin medio')+' · '+(m.cuenta||'Sin cuenta')+' · '+moneda;cuentas[k]=(cuentas[k]||0)+monto;operaciones[(m.referenciaTipo||m.origenTipo||'movimiento')+':'+(m.referenciaId||m.origenId||m.id)]=true;});var total=Object.keys(sumas).map(function(m){return posDinero(sumas[m],m);}).join(' · ')||'$0';return '<div class="sc-row"><div class="sc"><div class="scl">Caja de hoy</div><div class="scv cg">'+total+'</div></div><div class="sc"><div class="scl">Operaciones</div><div class="scv cb">'+Object.keys(operaciones).length+'</div></div></div><div class="pos-medios">'+Object.keys(cuentas).map(function(k){return '<span>'+esc(k)+' <b>'+posDinero(cuentas[k],k.slice(-3))+'</b></span>';}).join('')+'</div>';}

function posTituloMovimientoCaja(m){var origen=m.referenciaTipo||m.origenTipo||'movimiento';if(m.tipo==='ingreso_manual')return 'Ingreso manual'+(m.categoria?' · '+m.categoria:'');if(m.tipo==='egreso_manual')return 'Egreso manual'+(m.categoria?' · '+m.categoria:'');if(origen==='reparacion')return 'Cobro reparación '+(m.orden||'');if(m.tipo==='reversion_venta_equipo')return 'Anulación de equipo '+(m.equipoModelo||'');if(origen==='venta_equipo')return 'Venta de equipo '+(m.equipoModelo||'');if(m.tipo==='reversion_venta')return 'Anulación de venta';return 'Venta de accesorio';}
function posRenderMovimientosCaja(){var movs=window.MOVIMIENTOS_FINANCIEROS_POS||[];return '<div class="pos-section-title pos-mov-title">Movimientos de caja recientes</div><div class="pos-lista">'+movs.map(function(m){var titulo=posTituloMovimientoCaja(m);return '<div class="pos-list-row"><span><b>'+esc(titulo)+'</b><small>'+posFecha(m.fechaHora)+' · '+esc(m.medio||'')+' · '+esc(m.cuenta||'')+(m.clienteNombre?' · '+esc(m.clienteNombre):'')+(m.descripcion?' · '+esc(m.descripcion):'')+(m.moneda==='ARS'&&m.cotizacion?' · TC '+posDinero(m.cotizacion,'ARS'):'')+'</small></span><b class="'+(posNumero(m.monto)<0?'pos-out':'pos-in')+'">'+posDinero(m.monto,m.moneda)+'</b><small>'+esc(m.tipo||'')+'</small></div>';}).join('')+'</div>';}

function posRenderVentasLegacy(){var legacy=(window.VENTAS||[]).filter(function(v){return v.tipoRegistro!=='pos';});if(!legacy.length)return '';return '<div class="pos-section-title pos-mov-title">Ventas de equipos anteriores</div><div class="pos-lista">'+legacy.slice(0,100).map(function(v){return '<div class="pos-list-row"><span><b>'+esc(v.nombre||'Venta anterior')+(v.estadoVenta==='Anulada'?' · ANULADA':'')+'</b><small>'+esc([v.modelo,v.capacidad,v.fecha].filter(Boolean).join(' · '))+'</small></span><b>'+pesos(v.precio||0)+'</b><span><button class="btn btn-g btn-sm" data-vid="'+v.id+'" onclick="prtVenta(this.dataset.vid)">Comprobante</button>'+(puede('editar_ventas_equipos')?' <button class="btn btn-g btn-sm" data-vid="'+v.id+'" onclick="openEditVenta(this.dataset.vid)">Editar</button>':'')+(puede('eliminar_operaciones')&&v.estadoVenta!=='Anulada'&&v.estadoVenta!=='Devuelta'?' <button class="btn btn-d btn-sm" data-vid="'+v.id+'" onclick="anularVentaEquipo(this.dataset.vid)">Anular</button>':'')+'</span></div>';}).join('')+'</div>';}
