...
function posActualizarResultados(q) {
  ...
  box.innerHTML=posFiltrarProductos(q).map(function(p){
    ...
    return '<button class="pos-producto" type="button" data-pid="'+p.id+'" onclick="posAgregarProducto(this.dataset.pid)"><span><b>'+esc(p.nombre)+'</b><small>'+esc([p.sku,p.barcode,p.categoria].filter(Boolean).join(' · '))+'</small></span><span><b>'+pesos(p.precio)+'</b><small>'+stock+'</small></span></button>';
...
function posActualizarTotalesDom() {
  ...
  var e=el('posPagoDiferencia'); if(e){e.textContent=Math.abs(dif)<0.01?'Pago completo':(dif>0?'Faltan '+pesos(dif):'Excede '+pesos(-dif));e.className='pos-diferencia '+(Math.abs(dif)<0.01?'ok':'error');}
}
...
function renderPos() {
  var cnt=el('cnt'), c=posCalculos(), clientes={}, opcionesClientes=''; cnt.innerHTML='';
  ...
  cnt.innerHTML='<div class="pos-layout"><section class="pos-panel"><div class="pos-search"><input id="posBuscar" autocomplete="off" placeholder="Escanear código o buscar por nombre, SKU..." oninput="posActualizarResultados(this.value)" onkeydown="posBuscarInput(event)"><span>Enter para agregar</span></div><div id="posResultados" class="pos-resultados"></div></section>'
    ...
    +'<div class="pos-totales"><div><span>Subtotal</span><b>'+pesos(c.subtotalLista)+'</b></div><div><span>Descuentos</span><b>- '+pesos(c.descuentoTotal)+'</b></div><div class="total"><span>Total</span><b>'+pesos(c.total)+'</b></div></div>'
...
function posCobrar(imprimir) {
  ...
  var venta={ items:c.items.map(function(i){return {productoId:i.productoId,nombre:i.nombre,sku:i.sku,barcode:i.barcode,cantidad:i.cantidad,precioLista:i.precio,descuentoPorcentaje:i.descuentoPorcentaje,descuentoImporte:i.descuentoImporte,precioFinal:i.subtotalFinal/i.cantidad,costoUnitario:i.costo,costoTotal:i.costo*i.cantidad,controlaStock:i.controlaStock};}),
    pagos:POS_PAGOS.map(function(p){return {medio:p.medio,cuenta:p.cuenta,monto:posNumero(p.monto),moneda:'ARS',cotizacion:0};}),
    cliente:{nombre:(POS_CLIENTE.nombre||'Consumidor final').trim()||'Consumidor final',telefono:(POS_CLIENTE.telefono||'').trim()},
    moneda:'ARS',cotizacion:0,subtotal:c.subtotalLista,descuentoItems:c.descuentoItems,
...
function renderPosHistorial(){var cnt=el('cnt'),vs=posVentas(),hoyVentas=vs.filter(function(v){return v.estado==='activa'&&posEsHoy(v.fechaHora);}),total=hoyVentas.reduce(function(s,v){return s+posNumero(v.total);},0),unidades=hoyVentas.reduce(function(s,v){return s+(v.items||[]).reduce(function(a,i){return a+posNumero(i.cantidad);},0);},0),margen=hoyVentas.reduce(function(s,v){return s+posNumero(v.margenBruto);},0),medios={};hoyVentas.forEach(function(v){(v.pagos||[]).forEach(function(p){var k=p.medio+' · '+p.cuenta;medios[k]=(medios[k]||0)+posNumero(p.monto);});});cnt.innerHTML='<div class="sc-row"><div class="sc"><div class="scl">Vendido hoy</div><div class="scv cg">'+pesos(total)+'</div></div><div class="sc"><div class="scl">Operaciones</div><div class="scv cb">'+hoyVentas.length+'</div></div><div class="sc"><div class="scl">Productos</div><div class="scv cp">'+unidades+'</div></div>'+(puede('ver_costos')?'<div class="sc"><div class="scl">Margen estimado</div><div class="scv cg">'+pesos(margen)+'</div></div>':'')+'</div><div class="pos-medios">'+Object.keys(medios).map(function(k){return '<span>'+esc(k)+' <b>'+pesos(medios[k])+'</b></span>';}).join('')+'</div><div class="pos-lista">'+vs.map(function(v){return '<div class="pos-list-row pos-venta-row"><span><b>'+esc(v.numeroHumano||('Venta #'+v.numeroVenta))+'</b><small>'+posFecha(v.fechaHora)+' · '+esc((v.cliente&&v.cliente.nombre)||'Consumidor final')+' · '+esc((v.pagos||[]).map(function(p){return p.medio;}).join(' + '))+'</small></span><b class="'+(v.estado==='anulada'?'pos-anulada':'')+'">'+pesos(v.total)+'</b><button class="btn btn-g btn-sm" onclick="posVerVenta(\''+v.id+'\')">Ver</button></div>';}).join('')+'</div>'+posRenderVentasLegacy();}
...
function posVerVenta(id){var v=posVentas().find(function(x){return x.id===id;});if(!v)return;var body='<div class="pos-ticket-preview"><b>'+esc(v.numeroHumano||'Venta')+'</b><small>'+posFecha(v.fechaHora)+' · '+esc((v.cliente&&v.cliente.nombre)||'Consumidor final')+'</small>'+(v.items||[]).map(function(i){return '<div><span>'+i.cantidad+' × '+esc(i.nombre)+'</span><b>'+pesos(i.precioFinal*i.cantidad)+'</b></div>';}).join('')+'<div class="total"><span>Total</span><b>'+pesos(v.total)+'</b></div>'+(v.pagos||[]).map(function(p){return '<div><span>'+esc(p.medio)+' · '+esc(p.cuenta)+'</span><b>'+pesos(p.monto)+'</b></div>';}).join('')+(v.estado==='anulada'?'<p class="pos-anulada">ANULADA'+(v.anulacion&&v.anulacion.motivo?' · '+esc(v.anulacion.motivo):'')+'</p>':'')+'</div>';
